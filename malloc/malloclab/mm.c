...
*/
size_t user_malloc_size=0, heap_size;
...
void mm_free(void *bp)
{
    ...
    /*
    printf("%s, addr_start=%lu, size_head=%lu, size_foot=%lu\n",
    ...
    if(size > user_malloc_size)
    fprintf(test,"size = %lu user_malloc_size = %lu\n",size,user_malloc_size);//debug
    fclose(test);*/
    user_malloc_size -= GET_SIZE(HDRP(bp)) - WSIZE;
    coalesce(bp);
}
...
static void *coalesce(void *bp)
{
   ...
   size_t prev_prev;
    ...
    {
        prev_prev=GET_PREV_ALLOC(HDRP(PREV_BLKP(bp)));
        PUT(FTRP(bp),PACK(size+GET_SIZE(HDRP(PREV_BLKP(bp))),prev_prev,0));
        size+=GET_SIZE(HDRP(PREV_BLKP(bp)));
        PUT(HDRP(PREV_BLKP(bp)),PACK(size,prev_prev,0));
        bp=PREV_BLKP(bp);
    }
    else /*前后都是空闲块*/
    {
        ...
        PUT(HDRP(PREV_BLKP(bp)),PACK(size,prev_prev,0));
        bp=PREV_BLKP(bp);
        PUT(FTRP(bp),PACK(size,prev_prev,0));
    }
    return bp;
}
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    if((bsize-asize) >= 2*DSIZE){
        ...
        bp=NEXT_BLKP(bp);
        PUT(HDRP(bp),PACK(bsize-asize,1,0));
        PUT(FTRP(bp),PACK(bsize-asize,1,0));
        add_to_free_list(bp);
    }
    else{
        PUT(HDRP(bp),PACK(bsize,GET_PREV_ALLOC(HDRP(bp)),1));
        bp=NEXT_BLKP(bp); 
...
static void delete_from_free_list(void *bp)
{
    ...
    {
        /*mm_check(__FUNCTION__);*/
        /*printf("\nin delete from list: bp=%lu, prev_free_bp=%lu, next_free_bp=%lu\n", (size_t)bp, prev_free_bp, next_free_bp);*/
    }
...
void mm_check(const char *function)
{
    printf("\n---cur func: %s :\n", function);
     ...
     {
         count_empty_block++;
         printf("addr_start：%lu, addr_end：%lu, size_head:%lu, size_foot:%lu, PRED=%lu, SUCC=%lu \n", (size_t)bp - WSIZE,
