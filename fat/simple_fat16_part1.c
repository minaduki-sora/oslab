...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  if (findFlag == 1) {
    ...
    dir_entry_create(fat16_ins, sectorNum, offset, paths[pathDepth - 1], ATTR_ARCHIVE, 0xffff, 0);//未扩容
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
  ...
  if(FATClusEntryval != CLUSTER_END) printf("--FREE : %d\n",ClusterNum);//debug
...
int fat16_unlink(const char *path)
{
  ...
  if (findFlag == 1)
  {
    ...
    FILE * fd = fat16_ins->fd;
    BYTE secbuffer[BYTES_PER_SECTOR];
    sector_read(fd, sectorNum, secbuffer);
    secbuffer[offset] = buffer;
    sector_write(fd, sectorNum, secbuffer);
    /*
